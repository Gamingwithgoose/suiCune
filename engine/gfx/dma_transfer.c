...
static void HDMATransferAttrmapAndTilemapToWRAMBank3_Function(void){
    ...
    HDMATransferToWRAMBank3(0x0, wram->wScratchTilemap);
    // LD_A(0x1);
    // LDH_addr_A(rVBK);
    // LD_HL(wScratchAttrmap);
    // CALL(mHDMATransferToWRAMBank3);
    HDMATransferToWRAMBank3(0x1, wram->wScratchAttrmap);
    // RET;
}
...
static void Mobile_ReloadMapPart_Function(void){
    ...
    CopyBytes(gb.vram + VRAM_BANK_SIZE + (hram.hBGMapAddress & 0x1fff), wram->wScratchAttrmap, sizeof(wram->wScratchAttrmap));
    // LD_A(0x0);
    // LDH_addr_A(rVBK);
    // LD_HL(wScratchTilemap);
    // CALL(mHDMATransfer_NoDI);
    CopyBytes(gb.vram + (hram.hBGMapAddress & 0x1fff), wram->wScratchTilemap, sizeof(wram->wScratchTilemap));
    // POP_AF;
    // LDH_addr_A(rVBK);
    // NOP;
...
void HDMATransferToWRAMBank3(uint8_t vbk, uint8_t* hl) {
    // CALL(mv_LoadHDMAParameters);
    // LD_A(0x23);
    // LDH_addr_A(hDMATransfer);
    DelayFrame();
    CopyBytes(gb.vram + (VRAM_BANK_SIZE * vbk) + (hram.hBGMapAddress & 0x1ff0), hl, 1024); // (0x23 + 1) * 10 * 2);
}
